...
// #ifdef ALTO_DTHREADS
// #define NUM_THREADS(t) num_threads(t)
// #else
...
struct AltoTensor {
    ...
    int stream_mode;            // streaming mode
    int nprtn;                  // number of partitions
    IType nnz;                  // number of nonzeros
    IType max_nnz;              // number of nonzeros
...
template <typename LIT>
static inline void init_salto(SparseTensor *spt, AltoTensor<LIT> **at, int max_nprtn, int stream_mode);

template <typename LIT>
static inline void update_salto(SparseTensor *spt, AltoTensor<LIT> *at, int nprtn);

template <typename LIT>
static inline void update_salto_rowsparse(SparseTensor *spt, AltoTensor<LIT> *at, RowSparseMatrix **factors, std::vector<std::vector<int>>& ridx, std::vector<std::vector<size_t>>& oidx, int nprtn);
...
template <typename LIT>
static inline void mttkrp_alto_par(int target_mode, FType **factors, IType rank, AltoTensor<LIT> *at, omp_lock_t* wlocks, FType **ofibs, std::vector<std::vector<size_t>>& oidx);
...
template <typename LIT, typename ModeT, typename RankT>
static inline void
rowsparse_mttkrp_alto_stream(int const target_mode, RowSparseMatrix **factors, std::vector<std::vector<int>>& ridx, const AltoTensor<LIT>* const at, FType** ofibs, ModeT nmode = ModeT(), RankT rank = RankT());
...
#define SALTO_INIT_NNZ (1024*1024) // pre allocate ~1M NNZs    

// worst case da_mem cost = 4 mem ops
#define MIN_FIBER_REUSE 4
// small factor matrices
#define MIN_NUM_FIBERS 512
...
template <typename LIT, typename ModeT, typename RankT>
static inline void
mttkrp_alto_stream(int const target_mode, FType** factors, const AltoTensor<LIT>* const at, FType** ofibs, ModeT nmode = ModeT(), RankT rank = RankT());

template <typename LIT, typename ModeT, typename RankT>
static inline void
mttkrp_alto_op_sched(int const target_mode, FType** factors, const AltoTensor<LIT>* const at, std::vector<std::vector<size_t>>& oidx, ModeT nmode = ModeT(), RankT rank = RankT());
...
static inline void init_salto(SparseTensor *spt, AltoTensor<LIT> **at, int max_nprtn, int stream_mode)
{
    ...
    _at->nmode = nmode;
    _at->stream_mode = stream_mode;
    _at->nprtn = max_nprtn;
    _at->max_nnz = SALTO_INIT_NNZ;
    _at->nnz = SALTO_INIT_NNZ;
    _at->is_oidx = false;
    ...
    _at->idx = (LIT*)AlignedMalloc(_at->max_nnz * sizeof(LIT));
    assert(_at->idx);

    _at->vals = (FType*)AlignedMalloc(_at->max_nnz * sizeof(FType));
    assert(_at->vals);

    _at->cr_masks = (LIT*)AlignedMalloc(nmode * sizeof(LIT));
    assert(_at->cr_masks);

    _at->prtn_ptr = (IType*)AlignedMalloc((max_nprtn + 1) * sizeof(IType));
    assert(_at->prtn_ptr);

    _at->prtn_intervals = (Interval*)AlignedMalloc(max_nprtn * nmode * sizeof(Interval));
...
template <typename LIT>
static inline void
update_salto(SparseTensor* spt, AltoTensor<LIT>* at, int nprtn)
{
...
#ifdef ALTO_DTHREADS
    IType max_dim = _at->dims[0];
    for (int n = 1; n < nmode; ++n) {
        max_dim = std::max(max_dim, _at->dims[n]);
    }

    if (max_dim <= MIN_NUM_FIBERS) omp_set_num_threads(nprtn);
    else omp_set_num_threads(omp_get_max_threads());
#endif

    if (nnz > _at->max_nnz) { 
        _at->max_nnz = nnz;
        //realloc is not guranteed to be aligned
        _at->idx = (LIT*)realloc(_at->idx, nnz * sizeof(LIT));
        assert(_at->idx);

        _at->vals = (FType*)realloc(_at->vals, nnz * sizeof(FType));
        assert(_at->vals);
    }
...
template <typename LIT>
static inline void
update_salto_rowsparse(SparseTensor* spt, AltoTensor<LIT>* at, RowSparseMatrix **factors, std::vector<std::vector<int>>& ridx, std::vector<std::vector<size_t>>& oidx, int nprtn)
{
    double wtime_s, wtime;
    AltoTensor<LIT>* _at = at;
    int nmode = spt->nmodes;
    ...
    for (IType i = 0; i < nnz; i++) {
        ...
        _at->vals[i] = spt->vals[i];
        for (int j = 0; j < nmode; j++) {
            alto |= pdep(ridx[j][spt->cidx[j][i]], ALTO_MASKS[j]);
        }
        _at->idx[i] = alto;
    }
    wtime = omp_get_wtime() - wtime_s;
    ...
    bool sort_rowsparse = false; 
    for (int n = 0; n < nmode; ++n) {
        IType num_fibs = _at->dims[n];
        IType fib_reuse = _at->nnz / num_fibs;
        // fprintf(stderr, "ALTO: fib_reuse[%d]=%llu\n", n, fib_reuse);
        if (num_fibs > MIN_NUM_FIBERS && fib_reuse <= MIN_FIBER_REUSE) {
            sort_rowsparse = true;
            break;   
        }
    }//modes
    ...
    wtime_s = omp_get_wtime();
    if (sort_rowsparse) {
        size_t* oidx_idx = (size_t*)AlignedMalloc(nnz * sizeof(size_t));
        assert(oidx_idx);
        sort_alto_rowsparse(_at, oidx_idx);
        for (int n = 0; n < nmode; ++n) {
            IType num_fibs = _at->dims[n];
            IType fib_reuse = _at->nnz / num_fibs;
            if (num_fibs > MIN_NUM_FIBERS && fib_reuse <= MIN_FIBER_REUSE) {
                #pragma omp parallel for
                for (IType i = 0; i < nnz; ++i) {
                    oidx[n][i] = oidx_idx[oidx[n][i]];
                }        
            }
        }//modes        
        AlignedFree(oidx_idx);
    } else sort_alto(_at);
    wtime = omp_get_wtime() - wtime_s;
    fprintf(stderr, "ALTO: sort time = %f (s)\n", wtime);
...
create_da_mem(int target_mode, IType rank, AltoTensor<LIT>* at, FType*** ofibs)
{
...

    {
        ...
        #pragma omp parallel for reduction(+: total_storage) proc_bind(close) NUM_THREADS(nprtn)
        for (int p = 0; p < nprtn; p++) {
            ...
            if (target_mode == -1) {
                ...
                for (int n = 0; n < nmode; ++n) {
                    ...
                    if (at->dims[n] <= MIN_NUM_FIBERS || fib_reuse > MIN_FIBER_REUSE) {
                ...
                if (at->dims[target_mode] <= MIN_NUM_FIBERS || fib_reuse > MIN_FIBER_REUSE) {
...
destroy_da_mem(AltoTensor<LIT>* at, FType** ofibs, IType rank, int target_mode)
{
    ...
    #pragma omp parallel for NUM_THREADS(nprtn)
    for (int p = 0; p < nprtn; p++) {
        AlignedFree(ofibs[p]);
    }
#else
...
{
    void
    operator()(int target_mode, IType fib_reuse, FType** factors, IType rank, AltoTensor<LIT>* at, omp_lock_t* wlocks, FType** ofibs, std::vector<std::vector<size_t>>& oidx, MT nmodes)
    {
        if (at->dims[target_mode] > MIN_NUM_FIBERS && fib_reuse <= MIN_FIBER_REUSE) {
            // Use op_sched only if oidx is available
            if (at->is_oidx)
                return mttkrp_alto_op_sched(target_mode, factors, at, oidx, nmodes, rank);
        ...
        else if (target_mode == at->stream_mode)
            return mttkrp_alto_stream(target_mode, factors, at, ofibs, nmodes, rank);
        else
    ...
    operator()(int target_mode, IType fib_reuse, FType** factors, IType rank, AltoTensor<LIT>* at, omp_lock_t* wlocks, FType** ofibs, std::vector<std::vector<size_t>>& oidx, MT nmodes)
    {
        if (rank == Head) {
            ...
            if (at->dims[target_mode] > MIN_NUM_FIBERS && fib_reuse <= MIN_FIBER_REUSE) {
                // Use op_sched only if oidx is available
                if (at->is_oidx)    
                    return mttkrp_alto_op_sched(target_mode, factors, at, oidx, nmodes, const_rank());
                if (at->cr_masks[target_mode])
                ...
                return mttkrp_alto_stream(target_mode, factors, at, ofibs, nmodes, const_rank());
            else
                return mttkrp_alto_da_mem_pull(target_mode, factors, at, ofibs, nmodes, const_rank());
        }
        else
            return mttkrp_alto_rank_specializer<LIT, MT, 0, Tail...>()(target_mode, fib_reuse, factors, rank, at, wlocks, ofibs, oidx, nmodes);
    }
};
...
{
    void
    operator()(int target_mode, IType fib_reuse, FType** factors, IType rank, AltoTensor<LIT>* at, omp_lock_t* wlocks, FType** ofibs, std::vector<std::vector<size_t>>& oidx)
    { mttkrp_alto_rank_specializer<LIT, int, ALTO_RANKS_SPECIALIZED>()(target_mode, fib_reuse, factors, rank, at, wlocks, ofibs, oidx, at->nmode); }
};
...
struct mttkrp_alto_mode_specializer<LIT, 0, Head, Tail...>
{
    ...
    operator()(int target_mode, IType fib_reuse, FType** factors, IType rank, AltoTensor<LIT>* at, omp_lock_t* wlocks, FType** ofibs, std::vector<std::vector<size_t>>& oidx)
    {
        if (at->nmode == Head) {
            ...
            mttkrp_alto_rank_specializer<LIT, const_mode, ALTO_RANKS_SPECIALIZED>()(target_mode, fib_reuse, factors, rank, at, wlocks, ofibs, oidx, const_mode());
        }
        else
          mttkrp_alto_mode_specializer<LIT, 0, Tail...>()(target_mode, fib_reuse, factors, rank, at, wlocks, ofibs, oidx);
    }
};

template <typename LIT>
static inline void
mttkrp_alto_par(int target_mode, FType** factors, IType rank, AltoTensor<LIT>* at, omp_lock_t* wlocks, FType** ofibs, std::vector<std::vector<size_t>>& oidx)
{
    IType fib_reuse = at->nnz / at->dims[target_mode];
    return mttkrp_alto_mode_specializer<LIT, ALTO_MODES_SPECIALIZED>()(target_mode, fib_reuse, factors, rank, at, wlocks, ofibs, oidx);
}
...
template <typename LIT>
static inline void
sort_alto_rowsparse(AltoTensor<LIT>* at, size_t* oidx_idx)
{
    IType nnz = at->nnz;
    int nmode = at->nmode;
    ATrio<LIT>* at_trio = (ATrio<LIT>*)AlignedMalloc(nnz * sizeof(ATrio<LIT>));
    assert(at_trio);

    #pragma omp parallel for
    for (IType i = 0; i < nnz; ++i) {
        at_trio[i].oidx = i;
        at_trio[i].idx = at->idx[i];
        at_trio[i].val = at->vals[i];
    }

    std::sort(at_trio, at_trio + nnz, [](auto& a, auto& b) { return a.idx <b.idx; });

    #pragma omp parallel for
    for (IType i = 0; i < nnz; ++i) {
        oidx_idx[at_trio[i].oidx] = i;
        at->idx[i] = at_trio[i].idx;
        at->vals[i] = at_trio[i].val;
    } 
...
prtn_alto(AltoTensor<LIT>* at, int nprtn)
{
...
#ifdef ALTO_DEBUG
    fprintf(stderr, "nnz=%llu num_ptrn=%d, nnz_ptrn=%llu\n", nnz, nprtn, nnz_ptrn);
#endif
    ...
    at->prtn_ptr[0] = 0;
    #pragma omp parallel for schedule(static,1) proc_bind(close) NUM_THREADS(nprtn)
    for (int p = 0; p < nprtn; ++p) {
    ...
    for (int p = 0; p < nprtn; ++p) {
        ...
        for (int n = 0; n < nmode; ++n) {
            if (fib[n].start > fib[n].stop)
                fib[n].start = fib[n].stop;
...
// The code below finds the exact conflicts for each fiber in linear time (where nnz>> nptrn>> nmode).
    ...
    for (int n = 0; n < nmode; ++n) {
        ...
        fprintf(stderr, "ALTO: fib_reuse[%d]=%llu\n", n, fib_reuse);
        if ((at->dims[n] > MIN_NUM_FIBERS && fib_reuse <= MIN_FIBER_REUSE) && free_bits) {
            //if(free_bits){
...
mttkrp_alto_da_mem_pull(int const target_mode, FType** factors, const AltoTensor<LIT>* const at, FType** ofibs, ModeT nmode, RankT rank)
{
    ...
    #pragma omp parallel proc_bind(close) NUM_THREADS(nprtn)
    {
...
template <typename LIT, typename ModeT, typename RankT>
static inline void
mttkrp_alto_stream(int const target_mode, FType** factors, const AltoTensor<LIT>* const at, FType** ofibs, ModeT nmode, RankT rank)
{
    int const nprtn = at->nprtn;
    assert(nmode == at->nmode);
    ...
    #pragma omp parallel proc_bind(close) NUM_THREADS(nprtn)
    {
        ...
        for (int p = 0; p < nprtn; ++p) {
            ...
            FType* const out = ofibs[p];
            memset(out, 0, rank * sizeof(FType));
    ...
    //pull-based accumulation
    //without omp reduction due to its overhead
    for (int p = 0; p < nprtn; p++)
    {
        FType* const out = ofibs[p];
        #pragma omp simd
        for (IType r = 0; r < rank; r++) {
            factors[target_mode][r] += out[r];
        }
    } //prtns
}
...
template <typename LIT, typename ModeT, typename RankT>
static inline void
mttkrp_alto_op_sched(int const target_mode, FType** factors, const AltoTensor<LIT>* const at, std::vector<std::vector<size_t>>& oidx, ModeT nmode, RankT rank)
{
    int const nprtn = at->nprtn;
    assert(at->nmode == nmode);
    #pragma omp parallel for schedule(static,1) proc_bind(close) NUM_THREADS(nprtn)
    for (int p = 0; p < nprtn; ++p) {
        ...
        IType last_row;
 #ifndef ALT_PEXT
        last_row = pext(idx[oidx[target_mode][nnz_s]], ALTO_MASKS[target_mode]);
#else
        last_row = pext(idx[oidx[target_mode][nnz_s]], ALTO_MASKS[target_mode], at->mode_pos[target_mode]);
#endif
        bool is_boundry = true; 

        for (IType i = nnz_s; i < nnz_e; ++i) {
            FType const val = vals[oidx[target_mode][i]];
            LIT const alto_idx = idx[oidx[target_mode][i]];
...
struct rowsparse_mttkrp_alto_rank_specializer<LIT, MT, 0>
{
    ...
    operator()(int target_mode, IType fib_reuse, RowSparseMatrix** factors, std::vector<std::vector<int>>& ridx, IType rank, AltoTensor<LIT>* at, omp_lock_t* wlocks, FType** ofibs, MT nmodes)
    {
        if (at->dims[target_mode] > MIN_NUM_FIBERS && fib_reuse <= MIN_FIBER_REUSE) {
            ...
            return rowsparse_mttkrp_alto_stream(target_mode, factors, ridx, at, ofibs, nmodes, rank);
        else
    ...
    operator()(int target_mode, IType fib_reuse, RowSparseMatrix** factors, std::vector<std::vector<int>>& ridx, IType rank, AltoTensor<LIT>* at, omp_lock_t* wlocks, FType** ofibs, MT nmodes)
    {
        if (rank == Head) {
            ...
            if (at->dims[target_mode] > MIN_NUM_FIBERS && fib_reuse <= MIN_FIBER_REUSE) {
                ...
                return rowsparse_mttkrp_alto_stream(target_mode, factors, ridx, at, ofibs, nmodes, const_rank());
            else 
    ...
    for (int p = 0; p < nprtn; ++p) {
        ...
        for (IType i = nnz_s; i < nnz_e; ++i) {
            ...
            for (int m = 0; m < nmode; ++m) {
                if (m != target_mode) { //input fibers
                    ...
                    for (IType r = 0; r < rank; r++) {
                        ...
                        row[r] *= factors[m]->mat->vals[row_id * rank + r];
                    }
                }
            }
            ...
            for (IType r = 0; r < rank; ++r) {
                ...
                factors[target_mode]->mat->vals[row_id * rank + r] += row[r];
            }
        } //nnzs
    } //prtns
}
...
template <typename LIT, typename ModeT, typename RankT>
static inline void
rowsparse_mttkrp_alto_stream(int const target_mode, RowSparseMatrix** factors, std::vector<std::vector<int>>& ridx, const AltoTensor<LIT>* const at, FType** ofibs, ModeT nmode, RankT rank)
{
    int const nprtn = at->nprtn;
    assert(nmode == at->nmode);
    ...
    for (int p = 0; p < nprtn; p++)
    {
        ...
        #pragma omp simd
        for (IType r = 0; r < rank; r++) {
            factors[target_mode]->mat->vals[r] += out[r];
        }
    } //prtns
}
                ...
                if ((i >= offset) && (i <= stop)) {
                    ...
                    // } else {
                        ...
                        for (IType r = 0; r < rank; r++) {
                            ...
                            factors[target_mode]->mat->vals[i * rank + r] += out[j * rank + r];
                        }
...
{
    IType fib_reuse = at->nnz / at->dims[target_mode];
    // fprintf(stderr, "at->dims[%d] =%llu, fib_reuse=%llu\n", target_mode, at->dims[target_mode], fib_reuse); 
    // fprintf(stderr, "factors[%d] => nnz=%llu, fib_reuse=%llu\n", target_mode, factors[target_mode]->nnzr, at->nnz / factors[target_mode]->nnzr); 
