...
RowSparseMatrix * rspmat_init(IType nrows, IType ncols, IType nnzr)
{
    ...
    return (RowSparseMatrix *) rsp_mat;
}
...
RowSparseMatrix * convert_to_rspmat(Matrix * fm, size_t nnzr, size_t * rowind)
{
    ...
    #pragma omp parallel for schedule(static) //schedule(static, 1) if(nnzr > 50)
    ...
    //     #pragma omp simd
    //     for (IType r = 0; r < J; ++r) {
    //         rsp_mat->mat->vals[i * J + r] = fm->vals[row * J + r];
    //     }
    // }

    // Returned RSP matrix has same row indices as rowind
    ParMemcpy(rsp_mat->rowind, rowind, sizeof(size_t) * nnzr);
    return (RowSparseMatrix *) rsp_mat;
}
...
RowSparseMatrix* rsp_mat_mul(RowSparseMatrix * A, Matrix * B)
{
    ...
    ParMemcpy(rsp_mat->rowind, A->rowind, sizeof(size_t) * nnzr);
