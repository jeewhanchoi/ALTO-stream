...
void idxsort_hist(
  ...
  size_t* counts = (size_t*) AlignedMalloc(nthreads*sizeof(size_t));
    ...
    std::vector<size_t>& buckets)
{
    ...
    FType * const outmat = rsp_mats[mode]->mat->vals;
    ...
    // Write row index for rsp matrix
    for (IType i = 0; i < num_bins; ++i) {
        rsp_mats[mode]->rowind[i] = (size_t)X->cidx[mode][idx[buckets[i]]];
    }
