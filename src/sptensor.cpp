...
SparseTensor * AllocSparseTensor(const int nnz, const int nmodes) {
  SparseTensor * sp = (SparseTensor *) AlignedMalloc(sizeof(SparseTensor));
    sp->nnz = nnz;
    sp->nmodes = nmodes;

    sp->vals = (FType*)AlignedMalloc(nnz * sizeof(FType));
    sp->dims = (IType*)AlignedMalloc(nmodes * sizeof(IType));
    sp->cidx = (IType**)AlignedMalloc(nmodes * sizeof(IType*));
    for (int m = 0; m < nmodes; ++m) {
        sp->cidx[m] = (IType*)AlignedMalloc(nnz * sizeof(IType));
    }
    return sp;
}
